...
def test_rss_download(count=1):
    ...
    # 下载指定数量的新episode
    print(f"\n正在下载 {count} 个新episode...")
    episodes = parser.download_episodes(count=count)
...
def test_llm_processing(srt_path, txt_path, mp3_path=None, title=None):
    ...
    if mp3_path and segments_to_remove:
        print("\n步骤 4: 去除片头片尾，生成新MP3...")
        try:
            editor = AudioEditor()
            ...
            import traceback
            traceback.print_exc()
    elif mp3_path and not segments_to_remove:
...
def main():
    ...
    # 步骤1: RSS下载
    episode_data = test_rss_download(count=args.count)
